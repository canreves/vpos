...
def test_known_uat_card_behaviors_classify_success_and_quarantine_aliases() -> None:
    assert behavior_for_alias("akbank_visa_7068").status is CardAutomationStatus.SUCCESS_AUTO
    assert is_automatic_success_candidate("akbank_visa_7068") is True
    ...
    assert (
        behavior_for_alias("akbank_visa_5232").status
        is CardAutomationStatus.AUTOMATION_DIAGNOSTIC
    )
    assert is_automatic_success_candidate("akbank_visa_5232") is False
